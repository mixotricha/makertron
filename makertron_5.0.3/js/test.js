	...
	let reFormatModules = stream => { 
		...
		let splitArguments = (stream,start,end) => {
			...
			let consumeDepth = ( lft , rht , index , e ) => { 
				...
				( index <= e && e > 0 ) ? parameters[index] = "_"+parameters[index] : e = 0  
				if ( index < parameters.length ) consumeDepth( lft , rht , index + 1 , e ) 
			}
			consumeDepth( '(' , ')' , 0 , 0 )
			consumeDepth( '[' , ']' , 0 , 0 ) 
			consumeDepth( '{' , '}' , 0 , 0 ) 
			parameters = parameters.map( ( tkn , index ) => tkn === '=' ? ':' : tkn ) 
			...
			/*let splits=[], sliced = [] 
			//splits.push( start + 1 ) 
			let iterate = ( index , end , token_depth ) => {  
  			if ( (token_depth === 0 && stream[index] === ",")  ) {  
					if ( stream[index+2] === "=" ) splits.push(index+2) // pull only first level seperation ',' 
				}
    		if ( stream[index] === "[" || stream[index] === "(" ) token_depth++
    		if ( stream[index] === "]" || stream[index] === ")" ) token_depth--
				if ( index < end ) iterate(index+1,end,token_depth) 
  		}
			iterate(start,end,0) 
			splits.forEach( index => {	stream[index]=":"	})
	...
	let reformatFunCalls = stream => { 
		...
		let iterate = (index) => { 
			...
			if ( isToken(tkn) && !isExclude(tkn) && stream[index+1] === "(" ) {
				...
				if ( end !== -1 ) { 
					//result.push(  splitArguments( stream , index+2, end ) )
	...
	let scad = 'module foo ( s = 6 , j = 9 ) { union ( g = 3 , a = 4 ) { cube (j=k,m=12,size=[1,2,3],m=sin(20+6)); } difference() { for (x=[0:10:20]) rotate ([x,0,0,]) { translate([5,0,0]) { cylinder (size=5); } translate ([90,0,0]) rotate ([5,5,5]) sphere (r=5); } } }'
	...
	//let res = reformatFunCalls( reFormatModules( healClosure( src ) ) ) 

 
	//console.log( res.slice(28,54) ) 
	
	console.log( splitArguments(res,28,54) )
