...
var ThreeBSP = (function() {
	...
	ThreeBSP = function( pObject ) {
		...
		if ( pObject instanceof THREE.Geometry ) {
			// nothing
		} else if ( pObject instanceof THREE.Mesh ) {
			// #todo: add hierarchy support 
			
			pObject.updateMatrixWorld();
			
			var geometry = pObject.geometry; 

			var position = new THREE.Vector3();
			var quaternion = new THREE.Quaternion();
			var scale = new THREE.Vector3();
			pObject.matrixWorld.decompose( position, quaternion, scale );		
	
			var vert_length = geometry.vertices.length; 
			
			for ( i = 0; i < vert_length; i++ ) { 
				geometry.vertices[i].add( position ); // multiplying by position vector 
				geometry.vertices[i].applyQuaternion( quaternion ); // Applying rotation 
				// scale?
			}

		} else if ( pObject instanceof ThreeBSP.Node ) {
			this.tree = pObject;
			return this;
		} else {
